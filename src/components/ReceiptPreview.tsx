const lines = [
  { vendor: "CAMPUS BOOKSTORE", price: "118.00" },
  ...
  return (
    <div className="hidden select-none justify-self-end sm:block">
      <div className="relative w-64 rotate-[3deg] rounded-sm bg-paper-raised px-5 pb-8 pt-6 font-mono text-xs text-ink shadow-[0_18px_40px_-14px_rgba(27,24,18,0.35)] tear-edge">
          ...
          {lines.map((l) => (
            <div
              ...
              className={`flex items-baseline justify-between py-1 ${
                l.best ? "text-emerald-deep" : "text-ink-soft"
              }`}
            >
        ...
        <div className="pointer-events-none absolute -bottom-3 -right-3 rotate-[-8deg] rounded-sm border-2 border-emerald px-2.5 py-1 font-mono text-[11px] font-bold uppercase tracking-widest text-emerald opacity-80 animate-stamp">
          Buy now
        </div>
      </div>
    </div>
